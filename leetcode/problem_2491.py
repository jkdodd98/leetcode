...
skill of the ith player. Divide the players into n / 2 teams of size 2 such that the
total skill of each team is equal.

The chemistry of a team is equal to the product of the skills of the players on that
team.
...
class Solution:
    def dividePlayers(self, skill: list[int]) -> int:
        ...
        for i in range(1, len(skill) // 2):
            left, right = skill[i], skill[(i * -1) - 1]

            if (left + right) != score:
                return -1

            chemistry += left * right

        return chemistry


if __name__ == "__main__":
    Solution().dividePlayers(skill=[3, 4])
    Solution().dividePlayers(skill=[1, 1, 2, 3])
